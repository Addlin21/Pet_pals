...
alter table adoptionevents add column petid int;
set sql_safe_updates=1;
update adoptionevents set location='123 main st, cityville'  where eventid=1;
...
ALTER TABLE pets MODIFY petid INT NOT NULL AUTO_INCREMENT;

ALTER TABLE pets ADD COLUMN pettype VARCHAR(20);

ALTER TABLE pets DROP COLUMN pettype;

SHOW CREATE TABLE donations;
ALTER TABLE donations 
MODIFY donationid INT NOT NULL AUTO_INCREMENT;

SHOW TABLES LIKE 'adoptionevents';
SHOW CREATE TABLE participants;

ALTER TABLE participants MODIFY COLUMN participantid INT AUTO_INCREMENT;

SELECT eventid, eventname, eventdate, location
FROM adoptionevents
WHERE eventdate >= CURDATE() + INTERVAL 1 DAY;
